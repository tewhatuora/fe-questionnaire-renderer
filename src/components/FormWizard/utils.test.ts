...
it('builds the questionnaire initial values', async () => {
...
it('filters enabled items', async () => {
  const items = [
    {
      linkId: 'step3',
      prefix: 'step3.',
...
it('gets the current page inputs', async () => {
  expect(getCurrentPageInputs({ stepId: 'step1' }, {})).toEqual([])
  expect(
    getCurrentPageInputs({ stepId: 'step1000' }, {}, testQuestionnaire)
  ).toEqual([])
  expect(
    getCurrentPageInputs({ stepId: 'step3' }, {}, testQuestionnaire)
  ).toEqual([
    {
      linkId: 'step3.input6',
